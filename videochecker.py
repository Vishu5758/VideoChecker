...
def get_stream_props(path):
    ...
    if "/" in fr:
        ...
        framerate = round(num / den, 2) if den else 0
    else:
        framerate = float(fr) if fr else 0
    br = s.get("bit_rate")
    bitrate = f"{round(int(br) / 1000, 2)} Kbps" if br and br.isdigit() else None
    return {"width": w, "height": h, "codec": codec, "framerate": framerate, "bitrate": bitrate}
...
def check_ai(path, meta):
    keywords = [
        'dall-e','midjourney','stable diffusion','runway','synthesia',
...
missing = [t for t in (FFPROBE, FFMPEG) if not tool_exists(t)]
if missing:
    st.error(f"`{', '.join(missing)}` not found! Add to `apt.txt`:")
    st.code("ffmpeg")
    st.stop()
...
if st.button("Analyze"):
    ...
    c2.metric("Dur Pass",    pass_dur)
    c3.metric("Aud Fail",    fail_aud)
    c4.metric("Avg Duration", avg_str)
    ...
    st.json({k: info[k] for k in info if k not in ("Indicators",)})
    if info["Indicators"]:
